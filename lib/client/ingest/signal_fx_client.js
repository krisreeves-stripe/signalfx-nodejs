...
SignalFxClient.prototype._retrieveAWSUniqueId = function (callback) {
  ...
  request(getOptions, function (error, response, body) {
    if (error || response.statusCode !== 200) {
      callback(false, '');
      console.error('Failed to retrieve AWS unique ID: ', response ? response.statusCode : '', body);
    } else {
      try {
...
SignalFxClient.prototype.startAsyncEventSend = function () {
  ...
  while (this.rawEvents.length) {
    ...
    try {
      ...
      console.error('Can\'t process event: ', error);
    }
  }
};
...
SignalFxClient.prototype.post = function (data, postUrl, contentType) {
  ...
  return new Promise(function (resolve, reject) {
    request(postOptions, function (error, response, body) {
      if (error || response.statusCode !== 200) {
        console.error('Failed to send datapoint: ', response ? response.statusCode : '', body, error);
        reject(error);
      }
      resolve(body);
    });
